## KNN PT II: Using MNIST data for 28x28 pixel images
...
## ---- include = FALSE----------------------------------------------------
# To read .csv files:
read_digits_csv <- function(directory) {
  read.csv(directory, header = TRUE)
}

train <- read_digits_csv("data/train.csv")
test <- read_digits_csv("data/test.csv")

str(train)
dim(train)

## ------------------------------------------------------------------------
# Display the averages for all digits 0-9 of the observations
